...
FROM Tokens	IMPORT
  ...
  TokWith	, TokForeign	, TokRef	, TokExternal	;
...
BEGIN
  ...
  CASE Token OF
  | TokIdent		: pAttribute.Ident	:= NoIdent;
  | TokDecConst		,
    TokOctalConst	,
    TokHexConst		: pAttribute.IntValue	:= 1;
  | TokCharConst	: pAttribute.CharValue	:= CHR (0);
  | TokRealConst	: pAttribute.RealValue	:= DefaultReal;
  | TokStringConst	: pAttribute.StringValue:= DefaultString;
  ELSE ;
  END;
END ErrorAttribute;
}
...
START	Comment, Comment2, Str1, Str2
...
#STD, Comment2# "<*"	:- {INC (NestingLevel); yyStart (Comment2);}
#Comment2#  "*>"	:- {DEC (NestingLevel);
			    IF NestingLevel = 0 THEN yyStart (STD); END;}
#Comment2#  "<" | "*" | - {*<\t\n} + :- {}
...
#STD# REF		: {RETURN TokRef;}
#STD# EXTERNAL		: {RETURN TokExternal;}
